...
module.exports = async function (context, myTimer) {
  ...
  try {
    slackPostTS = (await (await slackBotChatPost(debugChannel,`${appName} (Every Wednesday @ 3:40pm)`)).json()).ts;
    ...
    if (isIdleDay({weekends_off:true, holidays_off:true})) {
      ...
      const PrResults = await doCovidStateDashboardTablesCasesDeaths(true);
