...
module.exports = async function (context, myTimer) {
  ...
  try {
    slackPostTS = (await (await slackBotChatPost(debugChannel,`${appName} (Every Friday @ 7:10am)`)).json()).ts;
    ...
    if (isIdleDay({weekends_off:true, holidays_off:true})) {
      ...
      const PrResults = await doCovidStateDashboardTablesCasesDeaths(false);
